...
$execute as @s at @s run function rr_recast:spells/core/functions/projectile/split_projectile/summon_splits with entity @n[type=marker,tag=rr.projectile.$(reference),tag=!rr.setup.$(reference).split] data.split